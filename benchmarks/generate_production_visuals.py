...
report_path = os.path.join(os.path.dirname(__file__), "COMPREHENSIVE_EXPERIMENT_REPORT.md")
with open(report_path, "r") as f:
    report_content = f.read()

df_train = parse_markdown_table(report_content, "Detailed Results (Training Weight Optimization)")
df_hpo = parse_markdown_table(report_content, "Detailed Results (Hyperparameter Optimization)")

# Professional Muted Palette
PALETTE = ['#4C72B0', '#DD8452', '#55A868', '#C44E52', '#8172B3', '#937860', '#DA8BC3']
COLORS = {cat: PALETTE[i % len(PALETTE)] for i, cat in enumerate(df_train["Category"].unique())}

def save_fig(name):
    plt.savefig(os.path.join(os.path.dirname(__file__), name), dpi=300, bbox_inches='tight')
    plt.close()

# --- FIG 1: Category Reliability (Box Plot) ---
plt.figure(figsize=(10, 6))
categories = [c for c in df_train["Category"].unique() if c != "Gradient"]
data_to_plot = [df_train[df_train["Category"] == cat]["Final Loss"].values for cat in categories]
...
df_hpo_norm["Algorithm"] = df_hpo_norm["Algorithm"].str.replace("SearchHT", "").str.replace("Search", "").str.replace("HT", "")
...
if df_merged.empty:
    ...
    df_hpo_norm["Algorithm"] = df_hpo_norm["Algorithm"].apply(lambda x: x.replace("Search", "").replace("HT", ""))
    df_train_norm["Algorithm"] = df_train_norm["Algorithm"].apply(lambda x: x.replace("Search", "").replace("HT", ""))
    df_merged = pd.merge(df_train_norm, df_hpo_norm, on=["Algorithm", "Category"])
...
for cat in categories:
    ...
    plt.scatter(sub["Norm_Loss"], sub["Best Accuracy"], label=cat.replace('_', ' ').title(), 
                color=COLORS.get(cat), s=100, alpha=0.8, edgecolors='white', linewidth=0.5)

top_gen = df_merged.sort_values(by="Combined", ascending=False).head(5)
for _, row in top_gen.iterrows():
    plt.annotate(row["Algorithm"], (row["Norm_Loss"], row["Best Accuracy"]), 
                 xytext=(5, 5), textcoords='offset points', fontsize=9, fontweight='medium')
...
wedges, texts, autotexts = plt.pie([superior, total - superior], 
                                  labels=['Outperformed Random', 'Underperformed Random'],
                                  autopct='%1.1f%%', startangle=140, 
                                  colors=['#55A868', '#C44E52'], explode=(0.05, 0))
...
plt.figure(figsize=(10, 6))
bars = plt.barh(top_15["Algorithm"], top_15["Best Accuracy"], 
