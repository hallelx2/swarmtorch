...
def run_full_training_benchmark():
    ...
    for baseline in ["Adam", "SGD"]:
        ...
        opt = optim.Adam(model.parameters(), lr=0.01) if baseline == "Adam" else optim.SGD(model.parameters(), lr=0.1)
        eval_count = 0
        final_loss = 0.0
        ...
        results.append({"Algorithm": baseline, "Category": "Gradient", "Final Loss": final_loss})
        ...
        for algo_name in algos:
            ...
            try:
                ...
                try:
                    ...
                    optimizer = opt_class(model.parameters(), population_size=SWARM_SIZE)
                ...
                results.append({"Algorithm": algo_name, "Category": category, "Final Loss": last_loss})
...
def run_full_hpo_benchmark():
    ...
    param_space = {'lr': (0.001, 0.1), 'hidden_dim': [16, 32, 64], 'dropout': (0.0, 0.4)}
    
    def build_fn(params): return HPOModel(int(params['hidden_dim']), params['dropout'])
    ...
    for category, algos in TUNE_MAP.items():
        ...
        for algo_name in algos:
            ...
            try:
                ...
                tuner = tuner_class(build_fn, param_space, train_fn, iterations=BUDGET_HPO, swarm_size=SWARM_SIZE, verbose=False)
                tuner.search()
                results.append({"Algorithm": algo_name, "Category": category, "Best Accuracy": 1.0 - tuner.best_score})
            except Exception as e:
                print(f"  FAILED {algo_name}: {e}")
...
if __name__ == "__main__":
    ...
    # Export Report
    report = f"# Comprehensive Research Benchmark Report\n"
    ...
    for cat in df_train["Category"].unique():
        if cat == "Gradient": continue
        best = df_train[df_train["Category"] == cat].sort_values(by="Final Loss").iloc[0]
