...
class TLBO(SwarmOptimizer):
    ...
    def _evaluate_fitness(self, particles: torch.Tensor, closure: Any = None) -> torch.Tensor:
        if closure is None:
            raise ValueError("TLBO requires a closure function")
    ...
    def _update_positions(self) -> None:
        ...
        for i in range(self.swarm_size):
            new_pos = self.positions[i] + torch.rand_like(self.positions[i]) * (teacher - tf * mean_pos)
            self.positions[i] = new_pos
            
            partner_idx = torch.randint(0, self.swarm_size, (1,)).item()
            if fitness[i] < fitness[partner_idx]:
                self.positions[i] += torch.rand_like(self.positions[i]) * (self.positions[i] - self.positions[partner_idx])
...
class HarmonySearch(SwarmOptimizer):
    ...
    def __init__(self, params: Any, swarm_size: int = 30, hmcr: float = 0.9, par: float = 0.3, device: str = "cpu") -> None:
        super().__init__(params, swarm_size=swarm_size, device=device, hmcr=hmcr, par=par)
        self.hmcr = hmcr
        self.par = par
        self.iteration_count = 0
    ...
    def _evaluate_fitness(self, particles: torch.Tensor, closure: Any = None) -> torch.Tensor:
        if closure is None:
            raise ValueError("HS requires a closure function")
        fitness = torch.zeros(particles.shape[0], device=self.device)
        for i in range(particles.shape[0]):
            self._set_params(particles[i])
            fitness[i] = closure().detach()
    ...
    def _update_positions(self) -> None:
        ...
        for i in range(self.swarm_size):
            if torch.rand(1, device=self.device).item() < self.hmcr:
                ...
                self.positions[i] = torch.rand(self.positions.shape[1], device=self.device)
